    ...
    export default function AddressPage() {
        ...
        const [editAddress, setEditAddress] = useState<AddressDivision | null>(null)
        ...
        // Hàm xử lý xóa
        const handleDeleteAddress = async (row: AddressDivision) => {
            if (!confirm(`Bạn có chắc muốn xóa ${row.name}?`)) return;

            try {
                await addressApi.deleteAddress(row.uid); // gọi API xóa
                ctoast.success("Xóa thành công!");

                // Cập nhật lại state dựa trên loại
                if (row.type === 'ward') {
                    setWards(prev => prev.filter(item => item.code !== row.code));
                } else if (row.type === 'street') {
                    setStreets(prev => prev.filter(item => item.code !== row.code));
                } else if (row.type === 'province') {
                    setProvinces(prev => prev.filter(item => item.code !== row.code));
                }
            } catch (err: any) {
                ctoast.error(err.message || "Xóa thất bại!");
            }
        };
        const handleEditAddress = (row: AddressDivision) => {
            setEditAddress(row)
            ...
            { key: 'isDeprecated', label: ' Ngừng sử dụng', render: row => row.isDeprecated ? 'Có' : 'Không' },
            {
                ...
                render: (row: AddressDivision) => (
                    <div className="flex space-x-2">
                        ...
                        { <button className="px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600"  onClick={() => handleDeleteAddress(row)}>Xóa</button> }
                    </div>
                ),
            }
        ]
        ...
        const handleSubmitEdit = async () => {
            ...
            try {
                const updated = await addressApi.updateAddress(editAddress.uid, { name: editName });

                ctoast.success("Cập nhật thành công!");

                if (updated.type === "province") {
                    setProvinces(prev => prev.map(p => p.uid === updated.uid ? updated : p));
                }
                else if (updated.type === "ward") {
                    setWards(prev => prev.map(w => w.uid === updated.uid ? updated : w));
                }
                else if (updated.type === "street") {
                    setStreets(prev => prev.map(s => s.uid === updated.uid ? updated : s));
                }
                ...
                ctoast.error(err.message || "Cập nhật thất bại!");
            }
        };
